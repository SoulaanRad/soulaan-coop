...
describe("ProposalEngine Features", () => {
  ...
  describe("Evaluation (replaces goalScores + scores)", () => {
  ...
  describe("Alternative Generation", () => {
    it("should generate alternatives array with overallScore", async () => {
      ...
      if (result.alternatives.length > 0) {
        ...
        expect(typeof alt?.overallScore).toBe("number");
      }
    });
  });
  ...
  describe("Missing Data Detection", () => {
    it("should identify missing data needs", async () => {
      const input = createInput("Want to open a store in downtown area.");
      const result = await engine.processProposal(input);
  ...
  describe("Decision Logic", () => {
    ...
    it("should include a decision with reason", async () => {
