...
contract SoulaaniCoin is ERC20, ERC20Pausable, AccessControlEnumerable {
    ...
    event VotingPowerCapChanged(
        uint256 oldPercent,
        uint256 newPercent,
        address indexed changedBy
    );

    event AwardLimitChanged(
        uint256 oldLimit,
        uint256 newLimit,
        address indexed changedBy
    );

    event SlashLimitChanged(
        uint256 oldLimit,
        uint256 newLimit,
        address indexed changedBy
    );
     ...
     */
    function setMaxVotingPowerPercent(uint256 newPercent) 
        external 
     ...
     * @dev Only callable by DEFAULT_ADMIN_ROLE (Treasury Safe)
     */
    function setMaxAwardPerTransaction(uint256 newLimit) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE) 
    {
     ...
     * @dev Only callable by DEFAULT_ADMIN_ROLE (Treasury Safe)
     */
    function setMaxSlashPerTransaction(uint256 newLimit) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE) 
    {
