...
🤖 Generated with Roadmap Bot`;
      ...
      const prCreateScript = `
cat > /tmp/autofix_pr_body.md << 'EOFBODY'
${body}
EOFBODY

gh pr create --title "${title}" --body-file /tmp/autofix_pr_body.md --label "needs-review"
rm /tmp/autofix_pr_body.md
`;
      
      const prUrl = execSync(prCreateScript, { encoding: 'utf8', shell: '/bin/bash' }).trim();
...
gh pr create --title "${title}" --body-file /tmp/pr_body.md --label "roadmap-update"
rm /tmp/pr_body.md
`;
      ...
      // Create detailed comment explaining the changes using file
...
gh pr comment ${prNumber} --body-file /tmp/pr_comment.md
rm /tmp/pr_comment.md
`;
      
      try {
        execSync(commentScript, { stdio: 'inherit', shell: '/bin/bash' });
